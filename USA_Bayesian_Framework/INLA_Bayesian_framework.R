...
## Notes:
##   
##   The code allows to estimate posteriors distributions of chosen set of model parameters using INLA
##   (INtegrated Laplace Approximation) package. This package computes posterior distribution by approximated
##   numerical integration instead of using MCMC to generate sample from the posterior distribution,
...
##   In the end of the code the 95% confidence intervals for values of each parameter are plotted for comparison.
##   
## ---------------------------
...
# load_default_model loads settings that define one out of five default models.
# Parameters include: formula, use_weekly, use_deaths, use_Poisson and output_folder (see ipynb notebook for further explanations).

load_default_model <- function(model) {
  if (model==1) {
    settings <- list(formula = new_cases ~ AQI_2weeks + popdensity +
  ...
  if (model==2) {
    settings <- list(formula = new_cases ~ AQI_2months + popdensity +
                       f(id, model = "iid") +  f(day, model = "ar1") + f(rowId, model = "iid"),
                     use_weekly = TRUE,
                     use_deaths = TRUE,
                     use_Poisson = TRUE)
  ...
  if (model==3) {
    settings <- list(formula = new_cases ~ AQI_2019 + popdensity +
                       f(id, model = "iid") +  f(day, model = "ar1") + f(rowId, model = "iid"),
                     use_weekly = TRUE,
                     use_deaths = TRUE,
                     use_Poisson = TRUE)
  ...
  if (model==4) {
    settings <- list(formula = new_cases ~ AQI_4years + popdensity +
                       f(id, model = "iid") +  f(day, model = "ar1") + f(rowId, model = "iid"),
                     use_weekly = TRUE,
                     use_deaths = TRUE,
                     use_Poisson = TRUE)
  ...
  if (model==5) {
    settings <- list(formula = new_cases ~ SO2 + O3 + PM2.5 + PM10 + NO2 + CO + popdensity +
                       f(id, model = "iid") +  f(day, model = "ar1") + f(rowId, model = "iid"),
                     use_weekly = TRUE,
                     use_deaths = TRUE,
                     use_Poisson = TRUE)
  }
  settings$output_folder <- paste("INLA_results//Model_", model, if (settings$use_deaths) "_deaths" else "_cases",
                                  if(settings$use_Poisson) "_poisson" else "_nbinomial", sep="")
  return(settings)
}

# run_Bayesian_model finds posterior distribution for given model parameters using Integrated Laplace Approximation 

run_Bayesian_model <- function(formula, use_weekly=TRUE, use_deaths=TRUE, use_Poisson=TRUE, output_folder) {
  ...
  # which are generated by INLA_data_preprocessing.R script.
  
  county_data <- read.csv("processed_data//county_data.csv")
  ...
  if (use_deaths) {
    ...
    # As in the original dataset the total number of deaths very occasionally decreases these cases are excluded.
    deathsTreshold <- 1
    ...
    # As in the original dataset the total number of cases very occasionally decreases these cases are excluded.
    casesTreshold <- 10
  ...
  covariates <- covariates[covariates %in% colnames(INLA_data)]
  INLA_data <- INLA_data[,c("id", "day", variables, covariates)]
  ...
  return(list(INLA_result=res, data_statistics=data_statistics))
}
...
plot_fixed_parameters <-  function(res, output_folder) {
  ...
  par(cex.lab=2, cex.axis=2, mar=c(4,5,2,2))
  ...
  axis(1, at=x, labels=rownames(fixed_parameters), cex.axis=2)
  
  dev.off()
  
  par(cex.lab=1, cex.axis=1, mar=c(4,5,2,2))
  plot(1, type="n", xaxt="n", xlab="Factor", ylab="Effect on infection rate", xlim=c(0.5,length(x)+0.5), ylim=c(min(low), max(high)))
  abline(a=0, b=0)
  rect(x - 0.2, low, x + 0.2, mid, col=color)
  rect(x - 0.2, mid, x + 0.2, high, col=color)
  axis(1, at=x, labels=rownames(fixed_parameters), cex.axis=1)
}

# Here the effect of each covariate on daily infection rate is plotted including 50% and 90% confidence intervals
# (represented as lighter and darker shaded area). The continuous line represents the median and dashed line represents
...
plot_covariates_effect <- function(res, data_statistics, use_weekly, output_folder, n_sims = 10000) {
  ...
  for (covariate in 1:(length(res$names.fixed)-1)) {
    ...
    ggplot(data_points) + aes(x = x, y = med) + geom_line() + geom_ribbon(aes(ymin = Q25, ymax = Q75), alpha = 0.2) +
      ...
      labs(x=res$names.fixed[covariate+1], y="Daily infection rate [%]") + theme(axis.text=element_text(size=24), axis.title=element_text(size=24))
    ggsave(paste(output_folder, "//", res$names.fixed[covariate+1], "_effect.png", sep=""), height=8, width=8)
    
    p <- ggplot(data_points) + aes(x = x, y = med) + geom_line() + geom_ribbon(aes(ymin = Q25, ymax = Q75), alpha = 0.2) +
      geom_ribbon(aes(ymin = Q10, ymax = Q90), alpha = 0.2) + geom_vline(xintercept = data_statistics[covariate,"mean"], linetype="dashed") +
