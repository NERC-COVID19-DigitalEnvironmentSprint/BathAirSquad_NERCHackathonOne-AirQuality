...
##
## Purpose of script: Preprocess pollutant data from 2019 to use in INLA_datasets_preprocessing.R
##
...
##   The output file pollution_data_2019.csv is used by the INLA_datasets_preprocessing.R script.
...
pollutant_data <- read.csv("annual_conc_by_monitor_2019.csv")
...
if (generatePlots) {
  for (i in pollutantsNames) {
    ...
    ggsave(paste(i, "_monitors_2019.png", sep=""), width = 10, height = 6)
  }
}
...
pollution_data_2019 <- read.csv("daily_aqi_by_county_2019.csv")
...
if (generatePlots) {
  ...
  png(height=800, width=800, file="Pollutant_correlation_plot.png", type = "cairo")
  corrplot(cor(variables), method = "circle")
  dev.off()
}

# Save the pollution_data_2019 in csv format. This file is used in INLA_datasets_preprocessing.R script.

write.csv(pollution_data_2019, "pollution_data_2019.csv", row.names=FALSE)
