...
# Model 5 describes the impact of each pollutant separately.
...
# (model="idd") are independent and identically distributed random variables, and model="ar1" is an autoregressive model of order 1).
# One can consider spatially autocorrelated model for id if the data were available for all counties
# (currently air pollution data for many counties is not available, and spatially autocorrelated model was unefficient)
...
if (model==3) {
  covariates <- c("AQI_2019", "popdensity")
...
if (model==5) {
  covariates <- c("SO2", "O3", "PM2.5", "PM10", "NO2", "CO", "popdensity")
  formula <- new_cases ~ SO2 + O3 + PM2.5 + PM10 + NO2 + CO + popdensity +
    f(id, model = "iid") +  f(day, model = "ar1") + f(rowId, model = "iid")
  use_weekly <- FALSE
}
...
axis(1, at=x, labels=rownames(fixed_parameters), cex.axis=1.3)
